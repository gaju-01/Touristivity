...
const Gok = (props) => {
  const [hr,min,sec]=useCounter({"hr":23,"min":12,"sec":45});
  const {err,image,title,content,single,execute}=useFetch();
  ...
  useEffect(()=>{
    execute({url:"https://webproject-backend-default-rtdb.firebaseio.com/Gokarna/Ohm%20beach.json"});
  },[execute]); 
  return (
    <div>
      <img
