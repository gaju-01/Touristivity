...
const Cart = () => {
  ...
  const {man_fun } =
    useManCart(setTot_Quan, setTot_Price,setCartlist,setErr);
  useEffect(() => {
                ...
                {data.image && (
                  <div key={data.title}>
                    ...
                    <form id={data.title} onSubmit={chngCart}>
                      <input className={cs["input-class"]} id={`input ${data.title}`} type="number" min="0" />
                      <Button className={cs["button-class"]} title="Submit" type="submit" />
                    </form>
