...
const useCart = () => {
  ...
  const [cart_listy,setCartList]=useState({title:"none",image:"",quantity:0,price:0,single:0});
  const backend = useCallback(async (config, details) => {
    ...
    const cart_list = data.cart_list;
    let found = false;
    ...
    if (!found) {
      const my_key = cart_list?cart_list.length:0;
    ...
    try{
      const fetch_back = await fetch(config.url_put, {
                          method: "PUT",
                          body: JSON.stringify({ tot_price:tot_price,tot_quan: tot_quan, cart_list: cart_list }),
                        });
      if(!fetch_back.ok){
        throw new Error();
      }
